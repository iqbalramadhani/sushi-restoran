...
export default function CategoryEdit({ category, errors }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        router.put(route('categories.update', category.id), form, { preserveScroll: true, onError: (err) => alert(Object.values(err).join('\n')) });
    };

    return (
        <>
            <Head title="Edit Kategori" />
            <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Edit Kategori</h2>}>
                <div className="py-6">
